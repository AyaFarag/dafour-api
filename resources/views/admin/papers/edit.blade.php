...
@section('content')
    <div class="box box-danger">
        ...
        <!-- /.box-header -->
        <!-- form start -->
        {!! Form::model($paper, ['route' => ['admin.papers.update', $paper->id], 'method' => 'post','files'=>true]) !!}
...
@section('js')
    ...
    <script>
        function appendToWrapper(wrapper, position = 0, title = '', link = '') {
            $(wrapper).append('<div class="row w-100 mb-3 ml-0 mr-0 blockinputs"><input value="' + title + '" type="text" placeholder="Reference Title" name="references[' + position + '][title]" class="form-control popups__input col-sm-6"><input value="' + link + '" type="url" placeholder="Reference Url" name="references[' + position + '][link]" class="form-control popups__input col-sm-6"><a class="float-right remove-reference">Remove Reference</a></div>');
        }
