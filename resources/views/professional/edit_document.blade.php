            ...
            <div class="m-auto m-0 p-0">
                <div class="popups__signing mt-4 mb-4 p-4 w-100 h-100 account-setting">
                    <div>
                        ...
                        {!! Form::model($paper, ['route' => ['professional.docs.update', $paper->id], 'method' => 'post', 'files' => 'true', 'class' => 'form-doc p-2']) !!}
...
@section('js')
<script>
    function appendToWrapper(wrapper, position = 0, title = '', link = '') {
        $(wrapper).append('<div class="row w-100 mb-3 ml-0 mr-0 blockinputs"><input value="' + title + '" type="text" placeholder="@lang("site.Reference Title")" name="references[' + position + '][title]" class="form-control popups__input col-sm-6"><input value="' + link + '" type="url" placeholder="@lang("site.Reference Url")" name="references[' + position + '][link]" class="form-control popups__input col-sm-6"><a class="float-right remove-reference">@lang("site.Remove Reference")</a></div>');
    }
    ...
    @if(isset($paper))
        @foreach($paper->references as $ref)
            appendToWrapper(wrapper, i, "{{ $ref->title }}", "{{ $ref->link }}");
            i++;
        @endforeach
    @endif
