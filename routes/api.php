...
Route::prefix('p')->group(function () {
    ...
    Route::post('register', 'Professional\Auth\RegisterController@register')->name('register');
// }

// Login {
    // Route::get('login', 'Auth\LoginController@showLoginForm')->name('login');
    Route::post('login', 'Professional\Auth\LoginController@login')->name('login');

    
    // Password {
        Route::get('password/reset', 'Auth\ForgotPasswordController@showLinkRequestForm')->name('password.reset');  // password rests // still
    ...
    // Authenticated {
        Route::group(['middleware' => [ 'professional', 'auth:professional']], function(){
            
            Route::get('confirm/{token}','Professional\ProfileController@confirm')->name('confirmation'); // still
            Route::post('confirm/phone', 'ProfileController@phoneConfirm') -> name('phone.confirm'); // still
            Route::post('confirm/phone/send', 'ProfileController@resendCode') -> name('phone.confirm.send'); // still
            
            Route::group(['middleware' => ['confirmed']], function(){

                    // Profile {
                    Route::post('profile/update', 'Professional\ProfileController@update')->name('profile.update'); 
                    // }
                    
                    // Documents {
                    // Route::get('docs/upload', 'DocumentsController@showUploadForm')->name('docs.upload');

                    Route::post('docs/upload', 'Professional\DocumentsController@upload');    // issue with store method

                    // Route::get('docs/{paper}/update', 'DocumentsController@showUpdateForm')->name('docs.update');

                    Route::post('docs/{paper}/update', 'Professional\DocumentsController@update'); 
...
Route::prefix('s')->group(function () {
        ...
        // Register {
        Route::post('register', 'Student\Auth\RegisterController@register')->name('register.student');
        // }
        ...
        // Logout {
        Route::get('logout', 'Auth\LoginController@logout')->name('logout.student');
        // }

    // Password {
        Route::get('password/reset', 'Auth\ForgotPasswordController@showLinkRequestForm')->name('password.reset'); // still
        Route::get('password/reset/{token}', 'Auth\ForgotPasswordController@showResetForm');
        Route::post('password/reset', 'Auth\ResetPasswordController@reset')->name('password.email');
        Route::post('password/email', 'Auth\ResetPasswordController@sendResetLinkEmail')->name('password.request');
    // }
// }
    ...
    // Authenticated {
        Route::group(['middleware' => ['student', 'auth:student']], function(){
            ...
            Route::get('confirm/{token}','Student\ProfileController@confirm')->name('confirmation'); // still
            Route::post('confirm/phone', 'ProfileController@phoneConfirm') -> name('phone.confirm');
            Route::post('confirm/phone/send', 'ProfileController@resendCode') -> name('phone.confirm.send');
            
            Route::group(['middleware' => ['studentconfirmed']], function(){

            // Authenticated user routes.
            // Route::get('home', 'HomeController@showHomePage')->name('home');

            // Profile {
            Route::post('profile/update', 'Student\ProfileController@update')->name('student.profile.update'); 
            // }
